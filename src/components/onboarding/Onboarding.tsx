...
const categories: Array<[Service["category"], string]> = [
  ["facial", "Facial"],
  ["corporal", "Corporal"],
  ["depilacao", "Depilação"],
...
const serviceSuggestions: Array<{
  title: string;
  category: Service["category"];
  duration: number;
}> = [
  { title: "Limpeza de pele", category: "facial", duration: 60 },
  {
    title: "Design de sobrancelhas",
    category: "sobrancelhas_cilios",
    duration: 45,
  },
  { title: "Manicure", category: "unhas", duration: 60 },
  { title: "Massagem relaxante", category: "massagem", duration: 60 },
  { title: "Depilação", category: "depilacao", duration: 30 },
  { title: "Escova", category: "cabelo", duration: 60 },
];
const field =
  "h-12 w-full rounded-xl border border-line bg-white px-4 font-body text-sm text-ink outline-none transition focus:border-ink focus:ring-4 focus:ring-ink/[.06]";
const label =
  "mb-2 block font-body text-[11px] font-medium uppercase tracking-[.08em] text-ash";
...
export function Onboarding({ user }: { user: User }) {
  ...
  const [step, setStep] = useState<Step>(
    [1, 2, 3, 4].includes(requested) ? (requested as Step) : 1,
  );
    ...
    Promise.resolve(
      ...
      .then(({ data }) => {
        ...
        setForm({
          ...
          businessType: data?.business_type || "autonoma",
      ...
      .catch(() => {
        if (active) {
          setNotice("Não foi possível carregar seu perfil. Tente novamente.");
          setLoading(false);
        }
      });
    return () => {
  ...
  const checkFirst = () => {
    if (!form.professionalName || cleanPhone(form.whatsapp).length < 10) {
      setNotice("Informe o nome da estética e um WhatsApp válido com DDD.");
      return;
    }
  ...
  const checkLocation = () => {
    if (!form.city || form.state.length !== 2 || !form.slug) {
      setNotice("Informe cidade, estado e um link para sua página pública.");
      return;
    }
    next();
  };
  const checkService = () => {
    if (
      ...
      (!form.serviceTitle ||
        ...
        (form.servicePriceType !== "on_request" && !form.servicePrice))
    ) {
      setNotice(
        "Escolha ao menos uma sugestão ou preencha seu primeiro serviço.",
      );
      return;
    }
    next();
  ...
  const finish = async () => {
    if (
      hours.some((hour) => hour.enabled && hour.end_time <= hour.start_time)
    ) {
      setNotice("O horário final precisa ser maior que o inicial.");
      return;
    }
    setSaving(true);
    setNotice("");
    try {
      const profilePayload = {
        user_id: user.id,
        full_name: form.professionalName,
        ...
        booking_auto_confirm: form.autoConfirm,
      ...
      const selected = serviceSuggestions.filter((item) =>
        selectedSuggestions.includes(item.title),
      );
      const custom = form.serviceTitle
        ? [
            {
              title: form.serviceTitle,
              description: form.serviceDescription,
              category: form.serviceCategory,
              duration_minutes: Number(form.serviceDuration),
              price_type: form.servicePriceType,
              price:
                form.servicePriceType === "on_request"
                  ? null
                  : Number(form.servicePrice.replace(",", ".")),
            },
          ]
        : [];
      await Promise.all(
      ...
      await replaceBusinessHours(
        ...
        hours
          ...
          .map(({ weekday, start_time, end_time }) => ({
            weekday,
            start_time,
            end_time,
          })),
      ...
      setNotice(
        error instanceof Error && error.message.includes("unique")
          ? "Este link já está sendo usado. Escolha outro."
      ...
      <div className="mx-auto max-w-[760px]">
        ...
        <section className="vello-material mt-10 rounded-[28px] p-5 shadow-[0_22px_70px_-48px_rgba(18,40,58,.34)] sm:p-9">
          ...
          {step === 1 && (
            <section className="mt-10">
              ...
              <h1 className="mt-3 font-display text-4xl font-semibold tracking-[-.055em]">
                Vamos apresentar sua estética.
              </h1>
              <p className="mt-3 max-w-xl font-body leading-relaxed text-ash">
                Essas informações aparecem na sua página pública e ajudam novas
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (file) setAvatarCandidate(file);
                  ...
                  <button
                    ...
                    className="mt-2 inline-flex items-center gap-1 font-body text-xs font-semibold underline underline-offset-4"
                  >
                    <ImagePlus size={13} />
              ...
              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                <label>
                  <span className={label}>
                    Nome da estética ou profissional
                  </span>
                  <input
                    className={field}
                    value={form.professionalName}
                    placeholder="Ex.: Studio Ana Rocha"
                    onChange={(event) => {
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      update("whatsapp", cleanPhone(event.target.value))
                    }
                  />
                </label>
                <label>
                  <span className={label}>Instagram · opcional</span>
                  <input
                    className={field}
                    value={form.instagram}
                    placeholder="@suaestetica"
                    onChange={(event) =>
              ...
              <p className={`${label} mt-7`}>Qual é o seu negócio?</p>
              <div className="grid gap-3 sm:grid-cols-2">
                {(
                  ...
                  <button
                    ...
                    className={`rounded-2xl border p-4 text-left transition ${form.businessType === value ? "border-ink bg-[#E8F1F8] ring-1 ring-ink" : "border-line hover:border-[#7EAFD0]"}`}
                  >
              ...
              <div className="mt-9 flex justify-end">
                <PrimaryButton onClick={checkFirst}>Continuar</PrimaryButton>
              </div>
            </section>
          )}
          {step === 2 && (
            <section className="mt-10">
              ...
              <h1 className="mt-3 font-display text-4xl font-semibold tracking-[-.055em]">
                Deixe sua página encontrável.
              </h1>
              <p className="mt-3 font-body leading-relaxed text-ash">
                Mostramos cidade e bairro. O endereço completo só aparece se
                você permitir.
              </p>
              <div className="mt-8 grid gap-5 sm:grid-cols-2">
                ...
                <label>
                  <span className={label}>Bairro · opcional</span>
                  <input
                    className={field}
              ...
              <label className="mt-5 flex items-center justify-between gap-4 rounded-2xl border border-line bg-[#F7FAFC] p-4 font-body text-sm">
                <span>
                  <b className="block">Mostrar endereço completo</b>
                  <small className="mt-1 block text-xs text-ash">
                    Deixe desligado se preferir informar apenas depois do
                    contato.
                  </small>
                </span>
                <input
                  ...
                  onChange={(event) =>
                    update("showAddress", event.target.checked)
                  }
                />
              </label>
              <label className="mt-7 block">
                <span className={label}>Seu link Vello</span>
                <div className="flex h-12 items-center overflow-hidden rounded-xl border border-line bg-white focus-within:border-ink">
                  <span className="shrink-0 border-r border-line bg-[#F7FAFC] px-3 font-body text-xs text-ash">
                    vello/
                  </span>
                  <input
                    className="min-w-0 flex-1 px-3 font-body text-sm outline-none"
                    value={form.slug}
                    onChange={(event) =>
                      update("slug", slugify(event.target.value))
                    }
                ...
                <small className="mt-2 block font-body text-xs text-ash">
                  {form.slug
                    ? publicLink(form.slug)
                    : "Escolha o endereço da sua página pública."}
                </small>
              </label>
              <div className="mt-9 flex items-center justify-between">
                ...
                <PrimaryButton onClick={checkLocation}>Continuar</PrimaryButton>
              </div>
            </section>
          )}
          {step === 3 && (
            <section className="mt-10">
              ...
              <p className="mt-3 font-body leading-relaxed text-ash">
                Escolha os serviços que você oferece e comece com o catálogo
                pronto. Você poderá ajustar preço, fotos e detalhes depois.
              </p>
              <div className="mt-7">
                <p className={label}>Sugestões para o seu catálogo</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  {serviceSuggestions.map((item) => {
                    const selected = selectedSuggestions.includes(item.title);
              ...
              <div className="mt-8 grid gap-5 sm:grid-cols-2">
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      update(
                        "serviceCategory",
                        event.target.value as Service["category"],
                      )
                    }
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      update("serviceDuration", event.target.value)
                    }
                  />
                </label>
                <label>
                  <span className={label}>Como cobrar</span>
                  <select
                    className={field}
                    value={form.servicePriceType}
                    onChange={(event) =>
                      update(
                        "servicePriceType",
                        event.target.value as Service["price_type"],
                      )
                    }
                  >
                ...
                {form.servicePriceType !== "on_request" && (
                  <label>
                    <span className={label}>Preço em R$</span>
                    <input
                      className={field}
                      inputMode="decimal"
                      ...
                      placeholder="120,00"
                      onChange={(event) =>
                        update("servicePrice", event.target.value)
                      }
                    />
                  </label>
                )}
              ...
              <label className="mt-5 block">
                ...
                <textarea
                  ...
                  placeholder="Conte para quem é indicado e o que está incluso."
                  onChange={(event) =>
                    update("serviceDescription", event.target.value)
                  }
                />
              </label>
              </>}
              <div className="mt-9 flex items-center justify-between">
                ...
                <PrimaryButton onClick={checkService}>Continuar</PrimaryButton>
              </div>
            </section>
          )}
          {step === 4 && (
            <section className="mt-10">
              ...
              <p className="mt-3 font-body leading-relaxed text-ash">
                Esses horários serão usados para disponibilizar a agenda online.
              </p>
              <div className="mt-8 space-y-3">
                {hours.map((hour, index) => (
                  <div
                    key={hour.weekday}
                    className="grid items-center gap-3 rounded-2xl border border-line p-3 sm:grid-cols-[1fr_120px_120px]"
                  >
                    ...
                    <input
                      type="time"
                      className="h-10 rounded-xl border border-line px-3 font-body text-sm disabled:opacity-40"
              ...
              <label className="mt-3 flex items-center justify-between gap-4 rounded-2xl border border-line bg-[#F7FAFC] p-4 font-body text-sm">
                <span>
                  ...
                  <small className="mt-1 block text-xs text-ash">
                    Desligue para aprovar cada pedido manualmente.
                  </small>
                </span>
                <input
                  type="checkbox"
                  ...
                  onChange={(event) =>
                    update("autoConfirm", event.target.checked)
                  }
                />
              </label>
              <div className="mt-9 flex items-center justify-between">
          ...
          {notice && (
            <p
              role="alert"
              className="mt-6 rounded-xl bg-red-50 px-4 py-3 font-body text-sm text-red-700"
            >
              {notice}
            </p>
          )}
