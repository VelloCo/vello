  ...
  return <>
    ...
    <div className="mt-9 grid items-start gap-8 xl:grid-cols-[minmax(0,.9fr)_minmax(430px,1.1fr)]">
      <div className="space-y-5">
        ...
        {hasChanges && <div className="sticky bottom-20 z-40 flex items-center gap-3 rounded-2xl border border-line bg-white/95 p-3 shadow-[0_12px_35px_rgba(11,11,10,.12)] backdrop-blur sm:bottom-5"><Button onClick={save} disabled={saving}>{saving ? 'Salvando...' : 'Salvar alterações'}</Button><button type="button" onClick={() => setTheme(savedTheme)} className="h-11 rounded-full px-4 font-body text-sm font-medium text-ash transition-colors hover:bg-cream hover:text-ink active:scale-[.98]">Descartar</button><span className="ml-auto hidden font-body text-xs text-stone sm:block">As mudanças só aparecem no catálogo após salvar.</span></div>}
      </div>
