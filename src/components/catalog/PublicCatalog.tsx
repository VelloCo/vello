  ...
  return (
    <>
      ...
      <main className="overflow-hidden bg-[#f5f2ec] pb-28">
        ...
        {contact && <section className="px-5 pb-16 sm:px-8 sm:pb-24"><div className="mx-auto max-w-[1400px] overflow-hidden rounded-[28px] bg-ink p-6 text-paper sm:rounded-[34px] sm:p-10 lg:p-12"><div className="border-b border-paper/15 pb-9 sm:flex sm:items-end sm:justify-between sm:gap-12"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-paper/45">Atendimento pessoal</p><h2 className="mt-5 max-w-xl font-display text-[42px] font-semibold leading-[.9] tracking-[-.055em] sm:text-6xl">Vamos encontrar o imóvel certo para você.</h2></div><p className="mt-6 max-w-sm font-body text-[16px] leading-relaxed text-paper/60 sm:mt-0">Tire dúvidas, peça mais fotos ou marque uma visita diretamente pelo WhatsApp.</p></div><div className="mt-6 flex items-center gap-3 rounded-[20px] bg-paper p-2.5 text-ink sm:mt-8 sm:rounded-full"><div className="h-11 w-11 shrink-0 overflow-hidden rounded-full bg-cream">{catalog.profile.avatar_url ? <img src={catalog.profile.avatar_url} alt="" className="h-full w-full object-cover" /> : <span className="grid h-full w-full place-items-center font-display text-base font-semibold">{catalog.profile.professional_name.slice(0, 1)}</span>}</div><div className="min-w-0 flex-1"><p className="truncate font-body text-sm font-semibold">{catalog.profile.professional_name}</p><p className="mt-0.5 truncate font-body text-[11px] text-ash">Corretor de imóveis{catalog.profile.creci ? ` · CRECI ${catalog.profile.creci.replace(/^CRECI\\s*/i, "")}` : ""}</p></div><a href={contact} target="_blank" rel="noreferrer" className="inline-flex h-11 shrink-0 items-center gap-2 rounded-full bg-ink px-4 font-body text-xs font-semibold text-paper transition hover:bg-charcoal sm:px-5 sm:text-sm"><MessageCircle size={16} />Conversar</a></div></div></section>}
      </main>
      {contact && (
        <a
          href={contact}
