...
function Header() {
  return <header className="border-b border-line bg-paper"><Container className="flex h-[68px] items-center justify-between"><a href="#inicio" aria-label="Vello - início"><Logo className="origin-left scale-[.78]" /></a><nav className="hidden items-center gap-8 font-body text-[14px] text-ash md:flex"><a href="#como-funciona" className="transition-colors hover:text-ink">Como funciona</a><a href="#catalogo" className="transition-colors hover:text-ink">Catálogo</a><a href="#perguntas" className="transition-colors hover:text-ink">Dúvidas</a></nav><a href={login} className="rounded-full border border-line px-4 py-2.5 font-body text-[13px] font-medium text-ink transition hover:border-ink active:translate-y-px">Entrar</a></Container></header>;
}

function Hero() {
  return <section id="inicio" className="border-b border-line bg-paper"><Container className="grid min-h-[calc(100dvh-68px)] gap-12 py-12 md:py-16 lg:grid-cols-[.8fr_1.2fr] lg:items-center lg:gap-16 lg:py-16"><Reveal y={14}><Eyebrow>Catálogo digital para corretores</Eyebrow><h1 className="mt-5 max-w-[580px] font-display text-[clamp(48px,5.5vw,76px)] font-semibold leading-[.91] tracking-[-.07em]">Imóveis bem apresentados. Conversas mais fáceis.</h1><p className="mt-7 max-w-[430px] font-body text-[17px] leading-relaxed text-ash">Organize imóveis, monte seu catálogo e compartilhe uma presença profissional em minutos.</p><div className="mt-9 flex flex-wrap gap-3"><a href={signup} className="inline-flex h-12 items-center gap-2 rounded-full bg-ink px-5 font-body text-[14px] font-semibold text-paper transition hover:bg-charcoal active:translate-y-px">Criar catálogo <ArrowRight size={16} /></a><a href="#como-funciona" className="inline-flex h-12 items-center rounded-full border border-line bg-white px-5 font-body text-[14px] font-medium transition hover:border-ink active:translate-y-px">Ver como funciona</a></div></Reveal><Reveal delay={.1} y={18}><HeroVisual /></Reveal></Container></section>;
}

function HeroVisual() { return <div className="overflow-hidden rounded-[20px] bg-cream shadow-[0_28px_80px_-46px_rgba(11,11,10,.38)]"><img src={appPath("/landing/vello-editorial-house.png")} alt="Casa contemporânea cercada por vegetação" className="aspect-[4/3] w-full object-cover" /></div>; }
