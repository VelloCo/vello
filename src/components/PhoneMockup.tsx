...
export function PhoneMockup({ children, className = '' }: { children?: ReactNode; className?: string }) {
  return (
    <div
      className={`relative w-[280px] rounded-[36px] border-[6px] border-ink bg-ink shadow-[0_50px_100px_-30px_rgba(18,40,58,0.55)] ${className}`}
    >
...
function CatalogList({ onOpen }: { onOpen: (id: string) => void }) {
  return <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.26, ease: [0.16, 1, 0.3, 1] }} className="h-full overflow-hidden bg-paper">
    <div className="bg-cream px-3 pb-4 pt-8">
      <div className="flex items-center gap-2.5 rounded-[18px] bg-white p-2.5 shadow-[0_12px_24px_-20px_rgba(18,40,58,0.55)] ring-1 ring-line/80">
    ...
    <div className="space-y-3 px-4 pb-6 pt-4">{properties.slice(0, 2).map((property) => <motion.button key={property.id} type="button" onClick={() => onOpen(property.id)} whileTap={{ scale: 0.985 }} className="block w-full overflow-hidden rounded-[16px] border border-line/70 bg-white text-left shadow-[0_10px_24px_-22px_rgba(18,40,58,0.5)]"><div className="h-[105px] bg-cover bg-center" style={{ backgroundImage: `url(${property.image})` }} /><div className="p-3"><p className="font-display text-[12.5px] font-medium leading-snug text-ink">{property.title}</p><p className="mt-0.5 font-mono text-[9.5px] text-stone">{property.neighborhood}</p><div className="mt-2 flex items-center justify-between"><p className="font-mono text-[13px] font-medium text-ink">{property.price}</p><span className="font-mono text-[8.5px] text-ash">{property.bedrooms}q · {property.parking}v · {property.area}m²</span></div><p className="mt-2.5 font-mono text-[8.5px] uppercase tracking-[0.12em] text-stone">Toque para ver detalhes →</p></div></motion.button>)}</div>
  </motion.div>;
}
